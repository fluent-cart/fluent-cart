...
class BulkEditModel extends ProductBaseModel {
    ...
    beforeInit() {
        ...
        this.data['constrainedProductIds'] = '';
        this.data['constrainedVariantIds'] = '';
        this.data['savedViews'] = [];
    ...
    async fetchProducts(filters = {}, append = false, productIds = '', variantIds = '') {
        this.data.loading = true;

        try {
            if (productIds) {
                this.data.constrainedProductIds = productIds;
                this.data.constrainedVariantIds = variantIds;
            }

            const idsToUse = productIds || this.data.constrainedProductIds;
            const variantsToUse = variantIds || this.data.constrainedVariantIds;
            ...
            if (idsToUse) {
                params.product_ids = idsToUse;
                params.variant_ids = variantsToUse;
            }
    ...
    setupSavedViews() {
        const tableConfig = window.fluentCartAdminApp?.table_config;
        this.data.savedViews = tableConfig?.products?.saved_views || [];
    }
    ...
    isViewOwner(viewLabel) {
        if (!viewLabel || !viewLabel.owner_id) return false;
        return viewLabel.owner_id === window.fluentCartAdminApp?.me?.id;
    }

    renameSavedView(viewId, newName) { /* no-op for BulkEdit */ }
    updateSavedView(viewId) { /* no-op for BulkEdit */ }
    deleteSavedView(viewId) { /* no-op for BulkEdit */ }
}
