<template>
  <div class="fct-revenue-comparison">
    <CardContainer class="overflow-hidden">
      <CardHeader :title="translate('Subscription Comparison')" title_size="small" />
    ...
    comparisonData() {
      ...
      this.currentMetrics.forEach((period, index) => {
        if (index < this.previousMetrics.length) {
          ...
          const currentCount = period.count;
          const previousCount = previousPeriod.count;
          ...
          comparisonData.push({
              ...
              previous_count: formatNumber(previousCount),
