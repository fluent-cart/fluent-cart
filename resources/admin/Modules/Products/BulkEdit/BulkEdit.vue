...
const loadBulkProducts = () => {
  const productIds = route.query.product_ids;
  const variantIds = route.query.variant_ids;

  if (productIds && variantIds) {
    bulkEditModel.fetchProducts({}, false, productIds, variantIds);
  } else {
    loadProducts(false);
  }
};
...
onMounted(async () => {
  ...
  bulkEditModel.setupSavedViews();

  // Fetch categories for picker
  bulkEditModel.fetchCategories();

  // Load products (either from inventory IDs or default)
  loadBulkProducts();
