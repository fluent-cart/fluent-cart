...
const props = defineProps({
  ...
  // Read-only title for advanced variations: { group, detail }.
  variantTitleInfo: Object,
})
const variant = ref({});
...
keyboardShortcuts.bind(['mod+s'], (event) => {
  // For `simple` type, EditProduct.vue's mod+s already persists the variant — saving here would emit a duplicate toast.
  if (props.product?.detail?.variation_type === 'simple') {
    return;
  }
...
const getVariantData = () => cloneVariant(variant.value);
