...
namespace FluentCart\Framework\Database\Query\Grammars;
...
class SQLiteGrammar extends Grammar
{
     ...
     *
     * @param  \FluentCart\Framework\Database\Query\Builder  $query
     * @param  \FluentCart\Framework\Database\Query\IndexHint  $indexHint
     * @return string
     */
    ...
    protected function compileUpdateColumns(Builder $query, array $values)
    {
        ...
        return Helper::collect($values)->reject(function ($value, $key) {
            return $this->isJsonSelector($key);
        })->merge($jsonGroups)->map(function ($value, $key) use ($jsonGroups) {
            $column = Helper::last(explode('.', $key));
    ...
    protected function compileUpdateWithJoinsOrLimit(Builder $query, array $values)
    {
        ...
        $alias = Helper::last(preg_split('/\s+as\s+/i', $query->from));
    ...
    public function prepareBindingsForUpdate(array $bindings, array $values)
    {
        ...
        $values = Helper::collect($values)->reject(function ($value, $key) {
            return $this->isJsonSelector($key);
