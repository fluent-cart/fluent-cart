...
class GutenbergEmailParser {
    ...
    private function renderBlocks($blocks, $nested = false) {
        ...
        // Debug: uncomment to see block structure
        // if (!$nested) {
        //     error_log('=== BLOCKS DEBUG ===');
    ...
    private function renderBlock($block) {
        ...
        switch ($blockName) {
            ...
            case 'core/audio':
                ...
                $url = $attrs['url'] ?? '';
                if (!empty($url)) {
                    $linkText = 'Click here to view media content';
                    return $this->wrapInTable("<p style=\"text-align: center; padding: 20px; background: #f0f0f0;\"><a href=\"{$url}\" style=\"color: #0073aa; text-decoration: underline;\">{$linkText}</a></p>");
                }
                return '';
    ...
    private function renderTable($content, $attrs) {
        ...
        if (isset($attrs['backgroundColor'])) {
            $tableStyles .= " background-color: {$attrs['backgroundColor']};";
        }
        if (isset($attrs['style']['color']['background'])) {
            $tableStyles .= " background-color: {$attrs['style']['color']['background']};";
        }

        // Handle text color
        if (isset($attrs['textColor'])) {
            $tableStyles .= " color: {$attrs['textColor']};";
        }
        if (isset($attrs['style']['color']['text'])) {
            $tableStyles .= " color: {$attrs['style']['color']['text']};";
        }
        ...
        if (isset($attrs['style']['typography'])) {
            ...
            if (isset($typography['fontSize'])) {
                $tableStyles .= " font-size: {$typography['fontSize']};";
            }
    ...
    private function getColorFromSlug($slug) {
        ...
        if ($colorMap === null) {
            ...
            $themeColors = (new \FluentCart\App\Services\Email\FluentBlockParser)->getThemeColorPalette();
            ...
            $themePref = (new \FluentCart\App\Services\Email\FluentBlockParser)->getThemePrefScheme();
