...
| File | Purpose |
|------|---------|
| `resources/admin/Modules/Products/BulkEdit/BulkEdit.vue` | Bulk Edit page (Vue SFC, no `<style>` section) |
| `resources/admin/Modules/Products/BulkInsert/BulkInsert.vue` | Bulk Insert page (Vue SFC, no `<style>` section) |
...
| `resources/styles/tailwind/bulk-insert.scss` | **Central stylesheet** (~950 lines) — all bulk table styling (Tailwind `@apply` + custom classes + dark mode) |
...
All styling lives in a **central SCSS file** — Vue components have **no `<style>` sections**.

**Central file:** `resources/styles/tailwind/bulk-insert.scss` (~950 lines)

Uses a hybrid approach: Tailwind `@apply` for base utilities, custom semantic classes for interactive states, raw CSS for pseudo-elements and animations.

### Input Styling (unified across both pages)
- `.bulk-cell-input` — custom input class (NOT Element Plus), uses `@apply` for Tailwind utilities
- Transparent bg by default, border on hover/focus via CSS vars (`--fct-border-gray-divider`, `--fct-text-system-dark`)
- `.bulk-editor-cell` — clickable description/excerpt cells, same min-height
- Error state: `.is-error` — red border `#F04438`, light red bg `#FEF3F2`

### Element Plus Overrides
- `.fct-bulk-input .el-input__wrapper` — focus border color override
- `.el-select` — full width, error state via `.is-field-error`
- `.bulk-price-input .el-input__wrapper` — custom padding/border
- All defined in the central SCSS file (no scoped `:deep()` in Vue)

### Row Styling
- Product rows: semantic class `.bulk-product-row` with conditional modifiers
- Variant rows: `.bulk-variation-row` + gray background
- `.is-dirty` — light gray bg (`bg-gray-50 !important`)
- `.is-saving` — opacity change for inputs
- `.is-saved` — opacity reduced, inputs disabled
- `.has-product-error` — error icon display
- `.is-selected` — gray background highlight
- `.is-link-mode` — dims non-selected rows to `opacity: 0.35`
- `.is-shift-held` — disables pointer events on inputs (shift+click selection)

### Status Icons (Edit only)
- `.bulk-status-icon` with states: `.is-syncing` (gray), `.is-saved` (green), `.is-error` (red)
- `.bulk-inline-save` — per-row save button in Title column

### Sticky Columns
- First column (Title) sticky left with `z-[65]`, `bg-white`
- `::after` pseudo-element with `linear-gradient()` for shadow on right edge
- Actions column optionally sticky right (`.sticky-col-right`)
- Both `th` and `td` get sticky treatment

### Dark Mode
- Full dark mode section (~250 lines) using `html.dark` selector
- Complete color remapping for backgrounds, text, borders, pseudo-element shadows
- Dark palette: `#1D2939`, `#283040`, text `#F2F4F7`, `#D0D5DD`

### Dynamic Sizing (ResizeableColumns.vue)
- Column widths, positions, z-index managed via inline `:style` bindings
- CSS custom properties: `--resizer-height`, `--header-height` set by ResizeObserver
- Table layout: `fixed` with computed total width
